...
#            print(sql_string)
        ...
        if len(user_public_osms)==0 and len(user_driving_osms)==0 and len(user_walking_osms)==0:
       ...
       FROM   (
            SELECT public_result.*,public_edges_2.transport_type,public_edges_2.transport_type_id
                     ...
                     FROM   (
                                   SELECT *
						 --public
                                   FROM   pgr_dijkstra( 'select id,source,target,x1,y1,x2,y2,cost,reverse_cost from public_edges_2', {user_public_array}, array
                                          (
...
#                    print(relevant_df.columns)
                    ...
                    results_dict[location][user]['node'] = relevant_df['node'].values.tolist()
                    results_dict[location][user]['start_vid'] = relevant_df['start_vid'].values.tolist()
                    results_dict[location][user]['end_vid'] = relevant_df['end_vid'].values.tolist()
