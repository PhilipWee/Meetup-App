...
xhr.onreadystatechange = (e) => {
 ...
 // result = xhr.responseText
  if (xhr.readyState == 4) {
    ...
    console.log(result)
    if ('info' in result) {
      //In future need to make this part nicer
      alert('The host has not confirmed the session! Please wait')
    }
    var list_of_restaurant_names = result['possible_locations']
    for (y in list_of_restaurant_names) {
      console.log(list_of_restaurant_names[y])
      ...
      document.getElementById("location_modifier" + String(list_of_restaurant_names[y])).addEventListener("click", function () {
        location_modifier(String(list_of_restaurant_names[y]))
      }, false);
    }
